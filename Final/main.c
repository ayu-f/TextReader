...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE: {
		...
		if (TextReader(&tx, (char*)structPar->lpCreateParams) == TRUE) {
	...
	case WM_KEYDOWN:
		...
		HandleArrows(wParam, hwnd);
		break;

	case WM_CHAR:
		// Handle keyboard characters
		HandleKeys(wParam, hwnd);
		break;
	case WM_COMMAND: {
		OPENFILENAME ofn;
		...
		case ID_FILE_OPEN: {
			...
			if (GetOpenFileNameA(&ofn)) {
				...
				if (TextReader(&tx, ofn.lpstrFile) == TRUE && !(VM_ViewInit(tx, &vw, hwnd) == TRUE && VM_ParseText(tx, &vw) == TRUE)) {
					FreeText(&tx);
